...
import { useGetCouponDataQuery, useGetDeletedCouponDataQuery, useRestoreCouponMutation, } from "../../../redux/features/catalogueEndPoints";

export default function RestoreCoupon() {
    const { isOpen, data, } = useSelector((state) => state.allCommonModal);
    const [restoreCoupon, { isLoading }] = useRestoreCouponMutation()
    const { refetch } = useGetCouponDataQuery();
    const { refetch: refchDeleted } = useGetDeletedCouponDataQuery();
    const dispatch = useDispatch();
    const handleRestore = async () => {
        try {
            const response = await restoreCoupon(data.id);
