...
import { useAddStaffMutation, useGetAllModulesQuery, useGetAllStaffQuery, useGetSingleStaffQuery, useUpdateSingleStaffPermissionMutation } from '../../redux/features/staffEndPoint';
...
export default function EditStaffPermissions() {
    ...
    const { data: userData, isLoading, isSuccess, refetch: singleStaffPermission } = useGetSingleStaffQuery(id);
    const [updateSingleStaffPermission, { isLoading: addLoading }] = useUpdateSingleStaffPermissionMutation();
    ...
    const { values, errors, handleBlur, touched, handleChange, handleSubmit, setFieldValue } = useFormik({
        ...
        onSubmit: async (values) => {
            ...
            try {
                const response = await updateSingleStaffPermission({ staffData: formData, staffId: id });
                if (response?.data?.http_status_code === 200) {
                    toast.success(response?.data?.message);
                    singleStaffPermission();
                    refetch();
    ...
    useEffect(() => {
        if (isSuccess) {
            ...
            setFieldValue("password", singleUserData.password);
    ...
    return (
        <>
            ...
            <Row>
                <Col>
                    <PageHeader titles="Staff Permissions" active="Edit Staff" items={["Home", "Staff List"]} links={["/dashboard", "/staff-permissions"]} />
                </Col>
            </Row>
            <Row>
                <Col>
