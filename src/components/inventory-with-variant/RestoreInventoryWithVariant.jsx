...
import { useGetAllInventoryWithVariantQuery, useGetTrashVariantQuery, useRestoreTrashVariantMutation } from "../../redux/features/stockInventoryEndPoint";
import { closeModal } from "../../redux/slices/allModalSlice";
import Loader from "../../layouts/layoutcomponents/loader";

export default function RestoreInventoryWithVariant() {
    const { isOpen, data, } = useSelector((state) => state.allCommonModal);
    console.log("data", data);
    const [RestoreInventoryWithVariant, { isLoading }] = useRestoreTrashVariantMutation()
    const { refetch } = useGetTrashVariantQuery();
    const { refetch: refchDeleted } = useGetAllInventoryWithVariantQuery();
    const dispatch = useDispatch();
    const handleRestore = async () => {
        try {
            const response = await RestoreInventoryWithVariant({ modelType: data.modelType, modelId: data.modelId.id });
