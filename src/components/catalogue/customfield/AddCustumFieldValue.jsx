...
import { AddCustomFieldValueSchema } from "../../../commondata/formvalidations";
import { Modal, Button, Form, Row, Col } from "react-bootstrap";
import { useSelector } from "react-redux";
import { useDispatch } from "react-redux";
import { closeModal } from "../../../redux/slices/allModalSlice";
import { useAddCustomFieldValuesMutation, useGetCustomFieldValueQuery } from "../../../redux/features/catalogueEndPoints";
import Loader from "../../../layouts/layoutcomponents/loader";
import toast from "react-hot-toast";



export default function AddCustumFieldValue() {
  const dispatch = useDispatch();
  const { isOpen, data } = useSelector((state) => state.allCommonModal);
  const { refetch } = useGetCustomFieldValueQuery(data?.data?.custom_field?._id);
  const [addCustomFieldValues, { isLoading }] = useAddCustomFieldValuesMutation()

  const initialValues = {
    custom_id: data?.data?.custom_field?._id,
    attribute_type: "",
    attribute_name: "",
    list_order: "",
    color: "",
  ...
  const { values, errors, handleBlur, touched, handleChange, handleSubmit, setFieldValue, } = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        const response = await addCustomFieldValues(values);
  ...
  const colorField = () => {
    ...
    if (data?.data?.custom_field.attribute_type === "Color" || data?.data?.custom_field.attribute_type === "color") {
      return (
        <>
    ...
    <div>
      <Modal show={isOpen}>
        <Modal.Header className="justify-content-center mx-2">
          <Modal.Title as="h4" className="fw-semibold lh-1 my-auto text-center">Add Custom Field Value</Modal.Title>
        </Modal.Header>
        ...
        <Modal.Body className="p-2">
          ...
          <Form onSubmit={handleSubmit} className="border rounded-2 p-2 overflow-hidden" >
            ...
            <Row className="mx-1">
              <Button type="submit" className="btn-primary ms-auto w-auto">Save</Button>
            </Row>
          </Form>
        </Modal.Body>
      </Modal>
