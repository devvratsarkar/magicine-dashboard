...
export default function ViewSurgicalEquipment() {
    const { id } = useParams()
    ...
    const { data, isError, error, isLoading, isFetching, isSuccess } = useGetSurgicalEquipmentIDQuery(id, { refetchOnMountOrArgChange: true })
    ...
    const { data: equipment, refetch } = useGetSurgicalEquipmentInventoryQuery(queryParams)
    ...
    const Linked_item_options = equipmentData ? equipmentData.map(device => ({
        value: device?.id,
    ...
    useEffect(() => {
        ...
        setFieldValue('linked_items', singleSurgicalEquipment && singleSurgicalEquipment?.linked_items.map(linkedItem => ({
            value: linkedItem?.id,
    ...
    if (isSuccess) {
        return (
            <>
                {isLoading && <Loader />}
                <PageHeader titles="Catalogue- Surgical Equipment" active="View surgical equipment" items={["Home", "Surgical Equipment List"]} links={["/dashboard", "/catalogue/surgical-equipment/"]} />
                <Row>
                    <Col md={12} lg={12}>
                        <Card>
                            <Card.Header>
                                <h3 className="card-title">View Surgical Equipment</h3>
                            </Card.Header>
                            <Card.Body className="add_new_product">
                                <Form onSubmit={handleSubmit}>
                                            ...
                                            <Row>
                                                <Col as={Col} md={3} className="d-flex justify-content-center" >
                                                    {viewThubnail ? <div className="position-relative">
                                                        <Link to={"/view-images"} state={{ data: singleSurgicalEquipment?.featured_image }}><img src={singleSurgicalEquipment?.featured_image} alt="error" width={50} height={50} /></Link>
                                    ...
                                    <Row className="mb-4">
                                        ...
                                        <Form.Group as={Col} md="4">
                                            ...
                                            <Form.Control
                                                ...
                                                disabled
                                            />
                                            {errors.gallery_img && touched.gallery_img ? (
                                                <p className={`error`}>{errors.gallery_img}</p>
                                            ) : null}
                                        </Form.Group>
                                        <Form.Group as={Col} md="4">
                                    ...
                                    <Row className="mb-4">
                                        <Form.Group as={Col} md={5} >
                                            ...
                                            <div className="manufacturer_btn">
                                                ...
                                                <Button type="button" disabled onClick={() => { dispatch(openModal({ componentName: 'AddNewManufacturer', data: 'check' })) }} className="btn btn-icon btn-primary" variant="primary"><i className="fe fe-plus"></i></Button>
                                            </div>                    {
                                    ...
                                    <Row>
                                        <Col as={Col} md={6}>
                                            <Form.Group>
                                                <Form.Label>SEO Discount <span className='text-danger'>*</span></Form.Label>
                                                <Form.Control
                                                    type="number"
                                                    name='seo_discount'
