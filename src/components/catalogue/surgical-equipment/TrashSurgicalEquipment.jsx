...
import { useGetTrashSurgicalEquipmentQuery, } from "../../../redux/features/catalogueEndPoints";

export const TrashSurgicalEquipment = () => {
    const [serialNumber, setSerialNumber] = useState(1)
    const [currentPage, setCurrentPage] = useState(1);
    const [pageSize, setPageSize] = useState(10);
    const [searchTerm, setSearchTerm] = useState("");
    const dispatch = useDispatch();
    const { data, isError, error, isLoading, isFetching, isSuccess } = useGetTrashSurgicalEquipmentQuery()
    ...
    if (isSuccess) {
                    ...
                    <div className="action_icon_wrapper d-flex justify-content-center">
                        {
                            showRestore && (
                                <OverlayTrigger placement="top" overlay={<Tooltip>Restore</Tooltip>}>
                                    <Button onClick={() => { dispatch(openModal({ componentName: 'RestoreSurgicalEquipment', data: row, })) }} className="btn btn-icon btn-warning" variant=""><i className="fa fa-refresh"></i></Button>
                                </OverlayTrigger>
                            )
                        }
                        {
                            showDelete && (
                                <OverlayTrigger placement="top" overlay={<Tooltip>Delete</Tooltip>}>
                                    <Button className="btn btn-con btn-danger border-danger" onClick={() => { dispatch(openModal({ componentName: 'DeleteSurgicalEquipment', data: row, softDelete: false })) }}> <i className="fe fe-trash"></i> </Button>
                                </OverlayTrigger>
                            )
                        }
                    </div>
        ...
        const filteredData = Array.isArray(productsData) ?
        ...
        return (
            <>
                <Row className="align-items-center">
                    <Col>
                        <PageHeader titles="Catalogue- Deleted Surgical Equipment" active="Trashed Surgical Equipment List" items={["Home", "Surgical Equipment"]} links={["/dashboard", "/catalogue/surgical-equipment/"]} />
                    </Col>
                </Row>
                <Card>
                    <Card.Body className="data_table">
                        <div className="e-table pb-5 table-responsive">
                            {loading && <Loader />}
                            <Row className="justify-content-end mt-3 mx-2 align-items-center mb-3">
