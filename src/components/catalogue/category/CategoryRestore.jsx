...
import { useGetCategoryDeletedItemsQuery, useGetCategoryQuery, useRestoreCategoryMutation } from "../../../redux/features/catalogueEndPoints";

export default function CategoryRestore() {
    ...
    const [restoreCategory, { isLoading }] = useRestoreCategoryMutation()
    const { refetch } = useGetCategoryQuery(categoryQuery);
    const { refetch: refchDeleted } = useGetCategoryDeletedItemsQuery();
    const dispatch = useDispatch();
    const handleRestore = async () => {
        try {
            const response = await restoreCategory({ categoryId: data.id });
