...
import {
  AddPushNotificationValidation,
  AddPushNotificationValidationWithouSchedule,
} from "../../../commondata/formvalidations";
import { options_for_sunEditor } from "../../../commondata/formEditorOptions";
import SunEditor from "suneditor-react";
import "suneditor/dist/css/suneditor.min.css";
import Select from "react-select";
import {
  useEditPushNotificationMutation,
  useGetAllPushNotificationQuery,
  useGetSinglePushNotificationQuery,
} from "../../../redux/features/supportDeskEndPoint";
import {
  useGetCategoriesTypeMutation,
  useGetAllUserAccordingToCategoryMutation,
} from "../../../redux/features/customerDataEndPoints";
...
function EditPushNotification() {
  ...
  const [editPushNotification, { isLoading: loading }] = useEditPushNotificationMutation();
  const {
    data,
    isError,
    error,
  ...
  } = useGetSinglePushNotificationQuery(id);
  const singlePushNotification = data?.data;
  ...
  }, [selectedProductType, SearchcategoryData]);
  ...
  }, [selectedCategory, searchUserData]);

  const handleSave = async () => {
    const { to, type, title, url, content, product_type, category } = values;
    const updatedValues = { to, type, title, url, content, product_type, category };
    try {
      const response = await editPushNotification({ pushNotificationData: updatedValues, pushNotificationId: id });
  ...
  const handleSchedule = async () => {
    const { to, type, title, url, content, date, time, product_type, category } = values;
    const schedule = `${date} ${time}`;
    const updatedValues = { to, type, title, url, content, schedule, product_type, category };
    try {
      const response = await editPushNotification({ pushNotificationData: updatedValues, pushNotificationId: id });
      if (response?.data?.http_status_code === 200) {
        toast.success(response.data.message);
  ...
  const userOptions =
    allUsers.length > 0
      ? [
        { value: "all_user", label: "Select All Users" },
        ...allUsers.map((user) => ({
          value: user.id,
          label: user.name,
        })),
  ...
  useEffect(() => {
    if (singlePushNotification) {
      setFieldValue("to", singlePushNotification.to?.map((item) => item.id));
      setFieldValue("type", singlePushNotification.type || []);
      setFieldValue("title", singlePushNotification.title);
      setFieldValue("url", singlePushNotification.url);
      setFieldValue("content", singlePushNotification.content);
      setFieldValue("product_type", singlePushNotification.product_type);
      setFieldValue("category", singlePushNotification.category);
      setSelectedProductType(singlePushNotification.product_type);
      setSelectedCategory(singlePushNotification.category);

      if (singlePushNotification.schedule) {
        const scheduleDate = new Date(singlePushNotification.schedule);
        setFieldValue("date", scheduleDate.toISOString().split("T")[0]);
        setFieldValue("time", scheduleDate.toISOString().split("T")[1].substring(0, 5));
        setSchedule(true);
      }
    }
  }, [singlePushNotification, setFieldValue]);
  ...
  return (
    <>
      {(isLoading || isFetching || loading || loadingCategory || loadingUsers) && <Loader />}
      <Row>
        <Col>
          <PageHeader
            titles="Support Desk"
            active="Edit Push Notification"
            items={["Home", "Push Notification List"]}
            links={["/dashboard", "/push-notification"]}
          />
        </Col>
        <Col className="my-5 text-end">
          <Link to="/push-notification" className="btn btn-success border-success">
            ...
            <Form onSubmit={handleSubmit}>
                        ...
                        onChange={(selectedOptions) => {
                          ...
                          if (allUsersSelected) {
                            selectedUserIds = allUsers
                              .filter((user) => user.value !== "all_user")
                              .map((user) => user.id);
                          } else {
                ...
                <Col as={Col} md={6}>
                  <Form.Group>
                    ...
                    <Select
                      ...
                      onChange={(selectedOptions) => {
                        const allTypeSelected = selectedOptions.some((option) => option.value === "All Type");

                        if (allTypeSelected) {
                          setFieldValue(
                            "type",
                            notificationType.map((type) => type.label),
                          );
                        } else {
                          setFieldValue(
                            "type",
                ...
                {/* <Col as={Col} md={6}>
                  <Form.Group>
                    ...
                    <Form.Control
                      ...
                      onBlur={handleBlur}
                    />
                    {errors.url && touched.url ? <p className="error">{errors.url}</p> : null}
                  </Form.Group>
                </Col> */}
              </Row>
              <Row>
                    ...
                    <Col as={Col} sm={3}>
                      <Form.Group>
                        ...
                        <Form.Control
                          ...
                          onBlur={handleBlur}
                        />
                        {errors.time && touched.time ? <p className="error">{errors.time}</p> : null}
                      </Form.Group>
                    </Col>
                  </Row>
                </Row>
              ...
              <Row className="mt-3 justify-content-center gap-2">
                {schedule ? (
                  ...
                  <p className="btn btn-primary w-auto" onClick={() => setSchedule(true)}>
                    Re-Schedule
                  </p>
                )}
                {schedule ? (
                  <p className="btn btn-primary w-auto" onClick={() => setSchedule(false)}>
                    Cancel
                  </p>
                ) : (
                  <Button type="submit" className="w-auto">
