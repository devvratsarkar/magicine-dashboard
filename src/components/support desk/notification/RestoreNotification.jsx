...
import { useGetAllNotificationQuery, useGetDeletedNotificationQuery, useRestoreNotificationMutation } from "../../../redux/features/supportDeskEndPoint";

export default function RestoreNotification() {
    const { isOpen, data, } = useSelector((state) => state.allCommonModal);
    const [restoreNotification, { isLoading }] = useRestoreNotificationMutation()
    const { refetch } = useGetAllNotificationQuery();
    const { refetch: refchDeleted } = useGetDeletedNotificationQuery();
    const dispatch = useDispatch();
    const handleRestore = async () => {
        try {
            const response = await restoreNotification({ notificationID: data.id });
