...
import { CMSShippingPolicyValidation, CMSTermsConditionValidation } from "../../../commondata/formvalidations";
...
import { useAddTermsConditionMutation, useGetTermsConditionQuery } from "../../../redux/features/cmsEndPoints";
import Loader from "../../../layouts/layoutcomponents/loader";
import { MEDIA_BASE_URL, USER_BASE_URL } from "../../../utils/config";


export default function CMSTermsCondition() {
  const [selectedThubnail, setSelectedThubnail] = useState(null);
  const [viewThubnail, setViewThubnail] = useState(true);
  const { data, isError, error, isLoading, isFetching, isSuccess, refetch } = useGetTermsConditionQuery()
  const [addShippingPolicy, { isLoading: loading }] = useAddTermsConditionMutation()

  const role = localStorage.getItem("role")
  const permissions = JSON.parse(localStorage.getItem("permissions"))

  const showButton = role === "Admin" || (role === "Staff" && permissions.Term_condition.includes("add"))
  ...
  const {
    ...
    validationSchema: CMSTermsConditionValidation,
    ...
    <>
      ...
      <Row className="justify-content-between">
        ...
        <Col className="my-5 text-end">
          {/* <Link className="btn btn-success">View All Pages</Link> */}
        </Col>
      </Row>
      <Row>
        <Col>
