...
import { useDeleteJobApplicationMutation, useGetAllJobApplicationQuery } from "../../../redux/features/cmsEndPoints";

export default function DeleteJobApplication() {
    const dispatch = useDispatch();
    const { isOpen, data } = useSelector((state) => state.allCommonModal);
    const [deleteApplication, { isLoading }] = useDeleteJobApplicationMutation();
    const { refetch } = useGetAllJobApplicationQuery();
    const handleDelte = async () => {
        try {
            const res = await deleteApplication(data.id)
