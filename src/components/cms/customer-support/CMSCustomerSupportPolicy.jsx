...
import { CMSCustomerSupportPolicyValidation } from "../../../commondata/formvalidations";
import { options_for_sunEditor } from "../../../commondata/formEditorOptions";
import Suneditor from "suneditor-react";
import "suneditor/dist/css/suneditor.min.css";
import SeoForm from "../../seo-page/SeoForm"
import { useAddCustomerSupportPolicyMutation, useGetCustomerSupportrPolicyQuery } from "../../../redux/features/cmsEndPoints"
import toast from "react-hot-toast";
import Loader from "../../../layouts/layoutcomponents/loader";
import { MEDIA_BASE_URL, USER_BASE_URL } from "../../../utils/config";

export default function CMSCustomerSupportPolicy() {
  const [selectedThubnail, setSelectedThubnail] = useState(null);
  const [viewThubnail, setViewThubnail] = useState(true);
  const { data, isError, error, isLoading, isFetching, isSuccess, refetch } = useGetCustomerSupportrPolicyQuery()
  const [addcustomerSupport, { isLoading: loading }] = useAddCustomerSupportPolicyMutation()

  const role = localStorage.getItem("role")
  const permissions = JSON.parse(localStorage.getItem("permissions"))

  const showButton = role === "Admin" || (role === "Staff" && permissions.Customer_support.includes("add"))
  ...
  const {
    ...
    validationSchema: CMSCustomerSupportPolicyValidation,
                  ...
                  <Col as={Col} md={6}>
                    <Form.Group>
                      <Row>
                        ...
                        <Col as={Col} md={6} className="text-end mt-3">
                  ...
                  <Col as={Col} md={6}>
                    <Form.Group>
                      ...
                      {/* {errors.video && touched.video ? (
                        <p className="error">{errors.video}</p>
                      ) : null} */}
                    </Form.Group>
                  </Col>
                </Row>
