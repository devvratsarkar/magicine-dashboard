...
import { CMSShippingPolicyValidation } from "../../../commondata/formvalidations";
...
import { useAddShippingPolicyMutation, useGetShippingPolicyQuery } from "../../../redux/features/cmsEndPoints";
import Loader from "../../../layouts/layoutcomponents/loader";
import { MEDIA_BASE_URL, USER_BASE_URL } from "../../../utils/config";

export default function CMSShippingPolicy() {
  const [selectedThubnail, setSelectedThubnail] = useState(null);
  const [viewThubnail, setViewThubnail] = useState(true);
  const { data, isError, error, isLoading, isFetching, isSuccess, refetch } = useGetShippingPolicyQuery()
  const [addShippingPolicy, { isLoading: loading }] = useAddShippingPolicyMutation()

  const role = localStorage.getItem("role")
  const permissions = JSON.parse(localStorage.getItem("permissions"))


  const showButton = role === "Admin" || (role === "Staff" || permissions.Shipping_policy.includes("add"))
  ...
  const {
    ...
    validationSchema: CMSShippingPolicyValidation,
    ...
    <>
      ...
      <Row className="justify-content-between">
        <Col>
          <PageHeader
            titles="CMS Page"
            active="Grievance Redressal Policy"
            items={["Home"]}
            links={["/dashboard"]}
          />
        </Col>
                  ...
                  <Col as={Col} md={6}>
                    <Form.Group>
                      ...
                      {errors.page_title && touched.page_title ? (
                        <p className="text-danger">{errors.page_title}</p>
                      ) : null}
                    </Form.Group>
                  </Col>
                  <Col as={Col} md={6}>
