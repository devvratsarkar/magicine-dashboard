...
import { useGetTestimonialsQuery, useGetTrashTestimonialsQuery, useRestoreTestimonialsMutation } from "../../../redux/features/cmsEndPoints";

export default function RestoreTestimonials() {
    const { isOpen, data, } = useSelector((state) => state.allCommonModal);
    const [restoreProduct, { isLoading }] = useRestoreTestimonialsMutation()
    const { refetch } = useGetTestimonialsQuery();
    const { refetch: refchDeleted } = useGetTrashTestimonialsQuery();
    const dispatch = useDispatch();
    const handleRestore = async () => {
        try {
            const response = await restoreProduct({ testimonialID: data.id });
