...
import brandImages from "../../assets/images/dashboard/image 144.png";
import slugify from "slugify";
import SeoForm from "../seo-page/SeoForm"
import { useEditBrandMutation, useGetBrandByIdQuery, useGetBrandQuery, useGetProductMedicineEquipmentQuery } from "../../redux/features/catalogueEndPoints";
...
export default function EditBrand() {
  ...
  const { id } = useParams();
  const { data, isError, error, isLoading, isFetching, isSuccess, refetch: refetchSingleBrand } = useGetBrandByIdQuery(id, { refetchOnMountOrArgChange: true })
  ...
  const { values, errors, handleBlur, touched, handleChange, handleSubmit, resetForm, setFieldValue, setFieldTouched, } = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        const response = await editBrand({ updatedData: formData, brandId: id });
  ...
  useEffect(() => {
    ...
    setFieldValue("shop_category_status", branData?.shop_category_status);
    ...
    setFieldValue("type", branData?.type || "Medicine")
  }, [isSuccess]);
  ...
  const generateMetaDescription = (productName, seoDiscount, type) => {
    let metaDescription;
    if (type === "General Product") {
      metaDescription = `Magicine Pharma Largest Supplier of ${productName}. Buy ${productName} Products Online at best price.`
    } else {
      metaDescription = `Magicine Pharma Largest Supplier of ${productName}. Buy ${productName} Medicines Online at best price.`
    }
  ...
  // useEffect(() => {
  //   generateMetaTitle(values.brand_name, values.seo_discount);
  //   generateMetaDescription(values.brand_name, values.seo_discount);
  // }, [values.brand_name, values.seo_discount]);
  ...
  const generateOgAndTwitterTag = () => {
    const ogTag = `
      ...
      <meta property="og:image" content="${branData && branData.featured_image ? branData.featured_image : `${MEDIA_BASE_URL}/public/media/images/1727083604842-354890b1-3d75-4ccd-a85a-0eebc8688c16.png`}">
      ...
      <meta name="twitter:image" content="${branData && branData.featured_image ? branData.featured_image : `${MEDIA_BASE_URL}/public/media/images/1727083604842-354890b1-3d75-4ccd-a85a-0eebc8688c16.png`}">
  ...
  useEffect(() => {
    ...
    if (branData && branData.featuredImage) {
      featuredImage = branData.featuredImage
    } else {
      featuredImage = `${MEDIA_BASE_URL}/public/media/images/1725347476566-487ac3b8-6b91-4c13-bc2f-40003050a634.png`;
    }
    ...
    return (
      <>
        <Row className="align-items-center">
          <Col>
            <PageHeader titles="Brand" active={["Edit Brand/"]} items={["Home", "Brand List"]} links={["/dashboard", "/brand"]} />
          </Col>
          <Col className="text-end"><Link to="/brand" className="btn btn-success text-white me-3">View All Brands</Link></Col>
        </Row>
        <Row>
          <Col>
            <Card>
              <Card.Header>Edit Brand</Card.Header>
              <Card.Body>
                {updating && <Loader />}
