...
import { useBrandRestoreMutation, useGetBrandQuery, useGetBrandsDeletedDataQuery } from "../../redux/features/catalogueEndPoints";

export default function BrandRestore() {
    const { isOpen, data, } = useSelector((state) => state.allCommonModal);
    const [queryBrand, setQuerybrand] = useState({
        type: "",
        status: ""
    })
    const [brandRestore, { isLoading }] = useBrandRestoreMutation()
    const { refetch } = useGetBrandQuery(queryBrand);
    const { refetch: refchDeleted } = useGetBrandsDeletedDataQuery();
    const dispatch = useDispatch();
    const handleRestore = async () => {
        try {
            const response = await brandRestore({ brandId: data.id });
