...
import { useGetAllCarrierQuery, useGetDeletedCarrierQuery, useRestoreCarrierMutation } from "../../../redux/features/shippingZoneEndPoints";

export default function RestoreCarrier() {
    const { isOpen, data, } = useSelector((state) => state.allCommonModal);
    const [restoreCarrier, { isLoading }] = useRestoreCarrierMutation()
    const { refetch } = useGetAllCarrierQuery();
    const { refetch: refchDeleted } = useGetDeletedCarrierQuery();
    const dispatch = useDispatch();
    const handleRestore = async () => {
        try {
            const response = await restoreCarrier(data.id);
