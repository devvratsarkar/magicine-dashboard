...
import Loader from "../../../layouts/layoutcomponents/loader";
import { closeModal } from "../../../redux/slices/allModalSlice";
import { useDeleteShippingMutation, useGetAllZonesQuery } from "../../../redux/features/shippingZoneEndPoints";

export default function DeleteZoneModal() {
    const dispatch = useDispatch();
    const { refetch } = useGetAllZonesQuery();
    const { isOpen, data, } = useSelector((state) => state.allCommonModal);
    const [deleteCategory, { isLoading }] = useDeleteShippingMutation();
    const handleDelte = async () => {
        try {
            const res = await deleteCategory({ url: data?.url, commonId: data?.id });
